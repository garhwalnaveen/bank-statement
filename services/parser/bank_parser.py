...
def row_to_transaction(row: list, schema: dict) -> Optional[Transaction]:
    ...
    # Handle explicit standalone DR/CR indicators in unmapped columns
    if t.amount is not None:
        dr_cr_explicit = None
        for k, v in raw.items():
            if str(v).strip().upper() in ("DR", "CR"):
                dr_cr_explicit = str(v).strip().upper()
                break
        
        if dr_cr_explicit == "DR":
            t.amount = -abs(t.amount)
        elif dr_cr_explicit == "CR":
            t.amount = abs(t.amount)
